...
from ktransformers.operators.flashinfer_wrapper import (
    flashinfer_enabled,
    MLAWrapperSingleton,
)
...
class KTransformersInterface(TransformersInterface):
    ...
    def prefill(
        ...
        if input_ids_length >= self.args.cache_lens:
            logger.warning(
                f"input_ids_length {input_ids_length} > cache_lens {self.args.cache_lens}"
            )
            self.seq_length = input_ids_length
            return
        logger.debug(f"input_ids: {input_ids.shape}")
        device = self.device_map.get("blk.0.self_attn", {}).get(
        ...
        self.seq_length += input_ids_length
        expected_length = min(
            self.seq_length + max_new_tokens + 1, self.args.cache_lens
        )
        ...
        if delta_length > 0:
            ...
            self.generated_ids = torch.cat(
                [self.generated_ids, new_generate_ids], dim=-1
            )
        else:
        ...
        self.generated_ids[:, cache_position] = input_ids.to(self.args.device).to(
            torch.int
        )
        ...
        self.max_new_tokens = (
            min(max_new_tokens, self.args.cache_lens - self.seq_length) - 1
        )
